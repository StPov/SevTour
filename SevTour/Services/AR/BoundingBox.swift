...
//  AR Navigator
//
//  Created by Dmitry Y. on 10/28/18.
//  Copyright © 2018 Dmitry Y. All rights reserved.
//
...
class BoundingBox {
    ...
    func showAlert(frame: CGRect, label: String, color: UIColor) {
        CATransaction.setDisableActions(true)
        ...
        textLayer.string = color == UIColor.red ? "STOP" : "GO"
        textLayer.backgroundColor = color.cgColor
        textLayer.isHidden = false
        ...
        let textOrigin = CGPoint(
            x: frame.origin.x + (frame.width / 2) - (textSize.width / 2),
            y: frame.origin.y + (frame.height / 2) - (textSize.height / 2)
        )
