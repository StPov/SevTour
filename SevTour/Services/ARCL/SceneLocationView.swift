...
public class SceneLocationView: ARSCNView, ARSCNViewDelegate {
    ...
    public func run() {
        ...
        updateEstimatesTimer?.invalidate() //10
        updateEstimatesTimer = Timer.scheduledTimer(timeInterval: 1000000, target: self, selector: #selector(SceneLocationView.updateLocationData), userInfo: nil, repeats: true)
    }
    ...
    public func session(_ session: ARSession, cameraDidChangeTrackingState camera: ARCamera) {
        ...
        case .limited(_):
            print("handled unknown camera tracking state")
        }
    }
}
